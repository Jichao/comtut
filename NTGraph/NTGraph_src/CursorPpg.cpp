...
void CCursorPropPage::AddCursor()
{
   ...
   LPCOLESTR lpOleStr = T2COLE(L"AddCursor" );
...
void CCursorPropPage::DeleteCursor(short annoID) 
{
   ...
   DISPID dwDispID;
   // Get a Unicode string
   LPCOLESTR lpOleStr = T2COLE(L"DeleteCursor" );
...
BOOL CCursorPropPage::OnInitDialog() 
{
	...
	int count;
    GetPropText(L"CursorCount", &count);
	...
	if(count>0)  
	{
		...
		{
			CString str;
			str.Format(L"Cursor-%d",i);
			m_lbCursors.AddString(str);
		}

		CString str;
		str.Format(L"%d",count-1);
		SetPropText(L"Cursor", str);
...
void CCursorPropPage::OnAddbutton() 
{
	...
	int index;
	GetPropText(L"Cursor", &index);
	
	str.Format(L"Cursor-%d",index);
	m_lbCursors.AddString(str);
    m_lbCursors.SetCurSel(index);
    
    SetPropText(L"Cursor", index);
	UpdateControls();
...
void CCursorPropPage::UpdateControls()
{
	...
	CString x,y;
	GetPropText(L"CursorX", &x);
    GetPropText(L"CursorY", &y);

	SetDlgItemText (IDC_EDIT_X, x);
	SetDlgItemText (IDC_EDIT_Y, y);

	COLORREF color;
	GetPropText(L"CursorColor", &color);
    m_btnColor.SetColour(color);

	int style;
	GetPropText(L"CursorStyle", &style);
	m_cbStyle.SetCurSel(style);

	int snap;
	GetPropText(L"CursorMode", &snap);
	m_cbSnap.SetCurSel(snap);


	BOOL visible;
	GetPropText(L"CursorVisible", &visible);
...
void CCursorPropPage::OnCheckVisible() 
{
	...
	if(count>0)
	{
		...
		if(m_btnVisible.GetCheck())
			SetPropText(L"CursorVisible", str);
		else
		{   
			str = "FALSE";
            SetPropText(L"CursorVisible", str);
...
void CCursorPropPage::OnCloseupComboStyle() 
{
	...
	if(count>0)
	{
		...
		SetPropText(L"CursorStyle", style);
	}		
}
