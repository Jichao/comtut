...
void CAnnoPropPage::AddAnnotation()
{
   ...
   LPCOLESTR lpOleStr = T2COLE(L"AddAnnotation" );
...
void CAnnoPropPage::DeleteAnnotation(short annoID) 
{
   ...
   DISPID dwDispID;
   // Get a Unicode string
   LPCOLESTR lpOleStr = T2COLE(L"DeleteAnnotation" );
...
void CAnnoPropPage::OnAddbutton() 
{
	...
	int index;
	GetPropText(L"Annotation", &index);
	
	str.Format(L"Annotation-%d",index);
	m_lbAnnotations.AddString(str);
    m_lbAnnotations.SetCurSel(index);
    
    SetPropText(L"Annotation", index);
	UpdateControls();
...
{
	CString caption;
	GetPropText(L"AnnoLabelCaption", &caption);
	SetDlgItemText (IDC_EDIT_CAPTION, caption);

	CString x,y;
	GetPropText(L"AnnoLabelX", &x);
    GetPropText(L"AnnoLabelY", &y);

	SetDlgItemText (IDC_EDIT_X, x);
	SetDlgItemText (IDC_EDIT_Y, y);

	COLORREF color;
	GetPropText(L"AnnoLabelColor", &color);
    m_btnColor.SetColour(color);

	GetPropText(L"AnnoLabelBkColor", &color);
    m_btnBkColor.SetColour(color);

	
	BOOL orientation;
	GetPropText(L"AnnoLabelHorizontal", &orientation);
	m_cbOrientation.SetCurSel(((int)orientation)+1);

	BOOL visible;
	GetPropText(L"AnnoVisible", &visible);
	...
	COLORREF colour;
	GetPropText(L"AnnoLabelColor", &colour);
...
BOOL CAnnoPropPage::OnInitDialog() 
{
	...
	if(count>0)  
	{
		...
		{
			CString str;
			str.Format(L"Annotation-%d",i);
			m_lbAnnotations.AddString(str);
		}

		CString str;
		str.Format(L"%d",count-1);
		SetPropText(L"Annotation", str);
...
void CAnnoPropPage::OnChangeEditCaption() 
{
	...
	if(count>0)
	{
		...
		SetPropText(L"AnnoLabelCaption", str);
	}
}
...
void CAnnoPropPage::OnCloseupComboOrientation() 
{
	...
	if(count>0)
	{
		...
		if(horizontal ==0)
			SetPropText(L"AnnoLabelHorizontal", str);
		else
		{
			str = "FALSE";
			SetPropText(L"AnnoLabelHorizontal", str);
...
LONG CAnnoPropPage::OnColorChange(UINT /*lParam*/, LONG /*wParam*/)
{
	...
	{
		CString str;
		str.Format(L"%d",m_btnColor.GetColour());
		SetPropText(L"AnnoLabelColor", str);	
	}	
...
LONG CAnnoPropPage::OnBkColorChange(UINT /*lParam*/, LONG /*wParam*/)
{
	...
	{
		CString str;
		str.Format(L"%d",m_btnBkColor.GetColour());
		SetPropText(L"AnnoLabelBkColor", str);	
	}	
