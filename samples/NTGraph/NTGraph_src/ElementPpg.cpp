...
void CElementPropPage::AddElement()
{
   ...
   LPCOLESTR lpOleStr = T2COLE(L"AddElement" );
...
void CElementPropPage::DeleteElement(long ElementID) 
{
   ...
   DISPID dwDispID;
   // Get a Unicode string
   LPCOLESTR lpOleStr = T2COLE(L"DeleteElement" );
...
void CElementPropPage::OnChangeEditWidth() 
{
	...
	SetPropText(L"ElementWidth", width);
}
...
void CElementPropPage::OnCheckSolid() 
{
	...
	SetPropText(L"ElementSolidPoint", solid );
}

void CElementPropPage::OnCheckVisible() 
{
	int visible = m_btnShow.GetCheck();
	SetPropText(L"ElementShow", visible );
...
LONG CElementPropPage::OnLineColorChange(UINT /*lParam*/, LONG /*wParam*/)
{
	...
	CString str;
	str.Format(L"%d", m_btnLineColor.GetColour());
	SetPropText(L"ElementLineColor", str);	
...
LONG CElementPropPage::OnPointColorChange(UINT /*lParam*/, LONG /*wParam*/)
{
	...
	CString str;
	str.Format(L"%d", m_btnPointColor.GetColour());
	SetPropText(L"ElementPointColor", str);	
...
void CElementPropPage::OnCloseupComboSymbol() 
{
   ...
   SetPropText(L"ElementPointSymbol", symbol );

}
void CElementPropPage::OnCloseupComboType() 
{
   int type = m_cbLineType.GetCurSel();
   SetPropText(L"ElementLinetype", type );
}
...
{
	COLORREF colour;
	GetPropText(L"ElementLineColor", &colour);
	m_btnLineColor.SetColour(colour);

	GetPropText(L"ElementPointColor", &colour);
	m_btnPointColor.SetColour(colour);

	int width;
	GetPropText(L"ElementWidth",&width);
    SetDlgItemInt(IDC_EDIT_WIDTH,width);

	int type;
	GetPropText(L"ElementLinetype", &type);
	m_cbLineType.SetCurSel(type);

	int symbol;
	GetPropText(L"ElementPointSymbol", &symbol);
	m_cbLineSymbol.SetCurSel(symbol);

	BOOL visible;
	GetPropText(L"ElementShow", &visible);
	...
	BOOL solid;
	GetPropText(L"ElementSolidPoint", &solid);
