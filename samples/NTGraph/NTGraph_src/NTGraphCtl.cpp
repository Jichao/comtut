...
void CNTGraphCtrl::DoPropExchange(CPropExchange* pPX)
{
	...
	PX_Picture(pPX, L"PlotAreaPicture",m_picPlotArea);
	PX_Picture(pPX, L"ControlFramePicture",m_picControlFrame);
}
		...
		if (nFlags==MK_LBUTTON) 
		{
			...
			switch (m_nTrackMode)
			{
				case Track:										
					...
					str.Format(L"x= %g y= %g",m_cursorX,m_cursorY);
...
void CNTGraphCtrl::PrintGraph()
{
	...
	if (hdcPrn != NULL)
	{
		...
		pDC->StartDoc(L"Graph2D"); // begin a new print job
...
void CNTGraphCtrl::DeleteElement(short ElementID) 
{
	...
	if (aPosition != NULL) {
		...
		AfxMessageBox(L"1 Element not found!") ;
...
void CNTGraphCtrl::PlotXY(double X, double Y, short ElementID) 
{
	...
	if (aPosition != NULL) {
		...
		AfxMessageBox(L"Element not found!") ;
...
void CNTGraphCtrl::PlotY(double Y, short ElementID) 
{
	...
	{
		dAutoRangeX[MIN]=std::floor(X);
		dAutoRangeY[MIN]=std::floor(X);
		dAutoRangeX[MAX]=std::ceil(X);
		dAutoRangeY[MAX]=std::ceil(X);
		bIsPlotAvailable= TRUE ;
	}
...
double CNTGraphCtrl::GetElementXValue(long index, short ElementID) 
{
	...
	if (aPosition != NULL) {
		...
		if(pos != NULL)
			...
			AfxMessageBox(L"Point not found!");
	} else
		AfxMessageBox(L"Element not found!") ;
   return 0.0;
}
...
void CNTGraphCtrl::SaveAs(LPCTSTR szFilename) 
{
  ...
  //Create the bitmap
  CreateBMPFile((LPTSTR)szFilename, pbi, (HBITMAP)bitmap.GetSafeHandle(), memDC);  
...
{ 
  ...
  {
    MessageBox(L"Unable to retrieve colour format, width and height of bitmap", L"", 
							MB_OK|MB_ICONERROR);
    return NULL;
  }
...
void CNTGraphCtrl::CreateBMPFile(LPTSTR pszFile, PBITMAPINFO pbi, HBITMAP hBMP, HDC hDC) 
{
  ...
  if(!lpBits) 
  {
    MessageBox(L"Unable to allocate memory for saving bitmap", L"", MB_OK|MB_ICONERROR);
    return;
  }
  ...
  {
    MessageBox(L"Unable to retrieve colour table when saving bitmap", L"", MB_OK|MB_ICONERROR);
    return;
  } 
  ...
  {
    MessageBox(L"Unable to open the file when saving bitmap", L"", MB_OK|MB_ICONERROR);
    return;
  }
  ...
  {
    MessageBox(L"Unable to write file header when saving bitmap", L"", MB_OK|MB_ICONERROR);
    return;
  }
  ...
  if(!WriteFile(hf, (LPVOID) pbih, sizeof(BITMAPINFOHEADER) 
                ...
                (LPDWORD) &dwTmp, ( NULL)) )
  {
    MessageBox(L"Unable to write bitmap info", L"", MB_OK|MB_ICONERROR);
    return;
  }
  ...
  {
    MessageBox(L"Unable to write colour indices when saving bitmap", L"", MB_OK|MB_ICONERROR);
    return;
  } 
  ...
  if (!CloseHandle(hf)) 
  {
    MessageBox(L"Unable to close file handle when saving bitmap", L"", MB_OK|MB_ICONERROR);
    return;
  } 
...
void CNTGraphCtrl::AddAnnotation() 
{
	...
	anno.m_Caption.Format(L"Annotation-%d", m_AnnotationList.GetCount());
...
CString CNTGraphCtrl::FormatXAxisLabel(double data)
{
  ...
  if(m_xLog)
		result.Format(L"1E%d",ROUND(data)); // Log Scale
  else if (m_xTime)
  {
